...
def co_abundance_profile(
    ...
    *,
    ...
    temperature : jnp.ndarray
        Temperature [K], broadcastable to N_h2.
    freezeout_K : float
    ...
    """
    warm = jnp.bitwise_and(temperature > freezeout_K, 0.706 * N_h2 > N_dissoc_cm2)

    if N_desorp_cm2 is None or jnp.isinf(N_desorp_cm2):
        cold = jnp.zeros_like(warm, dtype=bool)
    else:
        cold = jnp.bitwise_and(
            temperature <= freezeout_K,
            jnp.bitwise_and(0.706 * N_h2 > N_dissoc_cm2, 0.706 * N_h2 < N_desorp_cm2),
        )

    active = jnp.bitwise_or(warm, cold)
    return jnp.where(active, co_abundance, 0.0)
...
def velocity_profile(
    ...
    *,
    ...
    if pressure_correction:
        v = jnp.sqrt(
            vsq_keplerian_vertical(z, r, M_star_g) +
            vsq_pressure_grad(r, nd, temperature, m_mol_g)
        )
        return jnp.nan_to_num(v)
    else:
        return jnp.sqrt(vsq_keplerian_vertical(z, r, M_star_g))
...
def number_density_profile(
    ...
    *,
    ...
    """
    sigma_0 = (2.0 - gamma) * (M_gas_g / (2.0 * jnp.pi * (r_c_au * au) ** 2)) * jnp.exp(r_in_au / r_c_au) ** (2.0 - gamma)
    sigma   = sigma_0 * (r[0] / r_c_au) ** (-gamma) * jnp.exp(-(r[0] / r_c_au) ** (2.0 - gamma))

    dz      = jnp.diff(z * au, axis=0)
    dlogrho = (au**(-2) * G * M_star_g * z / (r**2 + z**2) ** 1.5) * (m_mol_g / (kk * temperature))
...
# Helpers
# ----------------------------------------------------------------------------- #
def vsq_keplerian_vertical(z: jnp.ndarray, r: jnp.ndarray, M_g: float) -> jnp.ndarray:
    """
    v_K^2 = G M r^2 / (r^2 + z^2)^(3/2), with r,z in AU then scaled to meters.
    ...
    """
    return G * M_g * (r * au) ** 2 / ((r * au) ** 2 + (z * au) ** 2) ** 1.5
...
def azimuthal_velocity(coords: jnp.ndarray, v_phi: jnp.ndarray) -> jnp.ndarray:
    """
    Lift scalar azimuthal speed to 3D velocity (vx, vy, vz=0) in the disk plane.
    """
...
# ----------------------------------------------------------------------------- #
co_abundance_profile_jit    = jax.jit(co_abundance_profile, static_argnames=("freezeout_K", "N_desorp_cm2", "co_abundance"))
